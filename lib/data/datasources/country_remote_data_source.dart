...
class CountryRemoteDataSourceImpl implements CountryRemoteDataSource {
        ...
        if (decodedData is Map<String, dynamic> && decodedData.containsKey('data')) {
          ...
          for (var item in objectsList) {
            if (item is Map<String, dynamic>) {
              try { countries.add(CountryModel.fromJson(item)); } catch (_) {}
            }
          }
          return countries;
        } else {
  ...
  Future<List<CountryModel>> searchCountries(String query) async {
    try {
      final url = 'https://api.restcountries.com/countries/v5/name?q=$query';
      ...
      throw Exception('Błąd wyszukiwania: $e');
    }
  }
}